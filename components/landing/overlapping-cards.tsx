        ...
        <div className="flex justify-center">
          <div className="relative flex flex-wrap justify-center gap-4 md:gap-0">
            {sampleInvoices.map((inv, i) => (
              <div
                key={inv.id}
                className={`glass-panel w-[280px] flex-shrink-0 p-5 transition-all hover:shadow-lg ${i === 1 ? "hover:border-[hsl(var(--accent))]/30 border-[hsl(var(--accent))]/15" : "hover:border-[hsl(var(--accent))]/20"}`}
                style={{
              ...
              >
                <p className="text-xs font-mono font-semibold text-[hsl(var(--accent))]">{inv.id}</p>
                <p className="mt-2 font-display text-lg font-bold text-foreground">{inv.debtor}</p>
                <p className={`mt-1 text-2xl font-display font-bold ${i === 1 ? "text-[hsl(var(--accent))]" : "text-foreground"}`}>{inv.amount}</p>
